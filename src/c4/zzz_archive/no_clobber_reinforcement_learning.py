...
from c4.no_clobber_env import NoClobberEnv, Color
...
if os.path.exists(model_path_1):
    ...
    dummy_env: NoClobberEnv = NoClobberEnv()
    common_params = dict(
        policy="MlpPolicy",
        env=dummy_env,
        learning_rate=0.001,
    ...
    for iteration in range(LEARNING_ITERATIONS):
        ...
        env1: NoClobberEnv = NoClobberEnv()
...
def execute_game(player1: BaseAlgorithm, deterministic: bool):
    board: NoClobberEnv = NoClobberEnv()
    board.reset()
